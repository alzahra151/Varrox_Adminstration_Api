...
async function AddDevice(data) {
    ...
    // console.log(data.ServiceID, device._id)
    await Service.findByIdAndUpdate(data.ServiceID, { $push: { Devices: device._id } });
    return device
}
...
async function DeleteDevice(id) {
    ...
    if (device == null) {
        ...
        try {
            ...
            await Service.update({ _id: device.ServiceID }, { $pull: { Devices: device._id } }, { session });
            // Commit the transaction
            await session.commitTransaction();
            return "Deleted Successfully"
        } catch (err) {
            await session.abortTransaction();
            console.error('An error occurred while deleting relational documents:', error);
