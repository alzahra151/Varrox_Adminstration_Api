...
async function getCompletedReqs() {
   const requests = await PriceOfferRequest.find({ HavePriceOffer: false, SendToAdmin: true, Complete: true, InitialAmountOfMoney: { $ne: null } }).populate('ReprsentativeID').populate('Services.Service').sort({ createdAt: -1 })
   return requests
}
